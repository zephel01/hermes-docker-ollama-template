...
Default mode runs Ollama on the host with no search stack. This is the right
choice for macOS, and the simpler choice for Linux unless you specifically
want everything inside Docker.
USAGE
}
...
if [ "$WITH_SEARCH" = "1" ]; then
  ...
  if ! grep -q '^SEARXNG_SECRET_KEY=' "$REPO_DIR/.env"; then
    SECRET="$(LC_ALL=C tr -dc 'a-f0-9' </dev/urandom 2>/dev/null | head -c 64 || \
              openssl rand -hex 32)"
    echo "" >> "$REPO_DIR/.env"
    echo "# Auto-generated by setup.sh --with-search" >> "$REPO_DIR/.env"
  ...
  # Install MCP config example as user mcp.yaml if missing.
  if [ ! -f "$HOME/.hermes/mcp.yaml" ]; then
    cp "$REPO_DIR/config/mcp.yaml.example" "$HOME/.hermes/mcp.yaml"
    echo "      Installed ~/.hermes/mcp.yaml (review before first run)"
  else
    echo "      ~/.hermes/mcp.yaml already exists (kept as-is)"
  fi
fi
...
[ "$WITH_SEARCH"   = "1" ] && COMPOSE_FILES="$COMPOSE_FILES -f compose.search.yml"
